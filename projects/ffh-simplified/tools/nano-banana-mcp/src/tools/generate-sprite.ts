...
export async function handleGenerateSprite(params: GenerateSpriteInput): Promise<ToolResult> {
  try {
    // Build the prompt
    ...
    return {
      content: [
        ...
        { type: 'image' as const, data: resizedBase64, mimeType: 'image/png' },
      ],
    };
  } catch (error) {
    if (error instanceof ApiKeyError) {
      return {
        content: [{ type: 'text' as const, text: 'Error: GEMINI_API_KEY environment variable is not set. Please set it to use this tool.' }],
        isError: true,
      };
    }
    if (error instanceof RateLimitError) {
      return {
        content: [{ type: 'text' as const, text: `Rate limited by Gemini API. Please retry after ${error.retryAfter} seconds.` }],
        isError: true,
      };
    }
    if (error instanceof GeminiApiClientError) {
      return {
        content: [{ type: 'text' as const, text: `Gemini API error (${error.status}): ${error.message}` }],
        isError: true,
      };
    }
    return {
      content: [{ type: 'text' as const, text: `Error generating sprite: ${(error as Error).message}` }],
      isError: true,
    };
  }
}
